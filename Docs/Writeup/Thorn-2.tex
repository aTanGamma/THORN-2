...
;...
    ...
    \section{Registers}\label{sec:Regs}
        ...
        For every RV64i operation, there are up to two operands, meaning up to two
            ...
            a register.
            To avoid using multiport RAM, two copies of the registers are needed:
            \textbf{x} and \textbf{y}; these two are both written to and read from
            at the same time. The \ramshort is 16-bit, so there are two sets of four
            RAM chips total.
        ...
        Some operations, such as conditional branches, compute a "result" which isn't
            needed. Whether such results are written to a register is tracked in the 
            CPU-wide control logic [LINK HERE] which issues the \textbf{WRT} signal.
        ...
        \begin{figure}[htb!]
            ...
            \begin{tikzpicture}[timing/picture, timing/unit=5mm]
                ...
                \timing[name=CLKTim] at (0,0) {
                    ...
                    \node[anchor=east](CLK_Label) at (CLKTim.west) {CLK};
                ...
                \timing[name=Adr1] at (0,-2) {
                    ...
                    \node[anchor=east](Adr1_Label) at (Adr1.west) {\(\mathrm{RS_{1}}\)[5]};
                ...
                \timing[name=Adr2] at (0,-4) {
                    ...
                    \node[anchor=east](Adr2_Label) at (Adr2.west) {\(\mathrm{RS_{2}}\)[5]};
                ...
                \timing[name=wAdr] at (0,-6) {
                    ...
                    \node[anchor=east](wAdr_Label) at (wAdr.west) {WB[5]};
                ...
                \timing[name=WrCtrl] at (0,-8) {
                    ...
                    \node[anchor=east](WrCtrl_Label) at (WrCtrl.west) {\(\mathrm{\overline{WE}}\)};
                ...
                \timing[name=WrCtrl] at (0,-10) {
                    ...
                    \node[anchor=east](WrCtrl_Label) at (WrCtrl.west) {\(\mathrm{\overline{OE_{X}}}\)};
                ...
                \timing[name=WrCtrl] at (0,-12) {
                    ...
                    \node[anchor=east](WrCtrl_Label) at (WrCtrl.west) {\(\mathrm{\overline{OE_{Y}}}\)};
                ...
                \timing[name=xAdr] at (0, -14) {
                    ...
                    \node[anchor=east](xAdr_Label) at (xAdr.west) {\(\mathrm{ADR_{X}}\)[5]};
                ...
                \timing[name=yAdr] at (0, -16) {
                    ...
                    \node[anchor=east](yAdr_Label) at (yAdr.west) {\(\mathrm{ADR_{Y}}\)[5]};
                ...
                \timing[name=xData] at (0, -18) {
                    ...
                    \node[anchor=east](xData_Label) at (xData.west) {\(\mathrm{X\_BUS}\) [64]};
                ...
                \timing[name=yData] at (0, -20) {
                    ...
                    \node[anchor=east](yData_Label) at (yData.west) {\(\mathrm{Y\_BUS}\) [64]};
                ...
                \timing[name=WrCtrl] at (0,-22) {
                    ...
                    \node[anchor=east](WrCtrl_Label) at (WrCtrl.west) {\(\mathrm{\overline{BUF\_EN_{X}}}\)};
                ...
                \timing[name=WrCtrl] at (0,-24) {
                    ...
                    \node[anchor=east](WrCtrl_Label) at (WrCtrl.west) {\(\mathrm{\overline{BUF\_EN_{Y}}}\)};
                ...
                \timing[name=yData] at (0, -26) {
                    ...
                    \node[anchor=east](yData_Label) at (yData.west) {\(\mathrm{REG\_OUT_{1}}\) [64]};
                ...
                \timing[name=yData] at (0, -28) {
                    ...
                    \node[anchor=east](yData_Label) at (yData.west) {\(\mathrm{REG\_OUT_{2}}\) [64]};
        ...
        \subsection{Control Logic}\label{sec:Regs:CtrlLogic}
            ...
            The design shown in \cref{fig:Regs:TheoryOp:StdRegFile} shows the control
                logic for the register file. 
            ...
            The design shown in \cref{fig:Regs:TheoryOp:StdRegFile} features two main
                ...
                busses, $\mathbf{D}_{Rs1}$ and $\mathbf{D}_{Rs2}$. 
                In $\phi_{1}$, both are dedicated to the writeback operation and the 
                read operation in $\phi_{2}$. 
                This occurs \emph{whether or not} the write is needed, the only 
                difference in the latter case being whether the \ramshort\ commits
                $\mathbf{D}_{WB}$ to storage (shown in \cref{tim:Regs:TheoryOp:RegTiming}).
            
            In \(\phi_{1}\), if \textbf{WRT} is high, a \(\overline{WE}\) controlled
                write cycle occurs with \(\overline{OE}\) set high. This is the simplest
                option given the signals needed.
            
            In \(\phi_{2}\), \(\overline{OE}\) is set low only if the read register is
                \emph{not} zero. In such a case, a register is read from as normal
                using an Address-controlled read cycle. When attempting to read from
                zero, resistors pull the data bus down, saving on parts over using
                buffers.

        \subsection{Operand Forwarding Disable}\label{sec:Regs:OpFwdDisEN}

            To make operand forwarding possible, the ALU may need an input from the
                registers, its own output, or a memory access. The registers therefore
                need to be able to be "selectable" by the CPU Control to pass their
                output on. The respective boards have buffered outputs which are enabled
                as needed.
